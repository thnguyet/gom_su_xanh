...
Start-Process powershell -ArgumentList "-NoExit -Command `"cd be/discovery-service; mvn spring-boot:run`""
...
foreach ($service in $services) {
    if ($service -ne "discovery-service") {
        ...
        Start-Process powershell -ArgumentList "-NoExit -Command `"cd be/$service; mvn spring-boot:run`""
...
# Chạy server tại thư mục gốc để truy cập được cả fe-user và fe-admin
Start-Process powershell -ArgumentList "-NoExit -Command `"npx http-server . -p 3000 -c-1 -o /fe-user/pages/trang-chu_417-354.html`""
